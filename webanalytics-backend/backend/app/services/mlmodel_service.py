...
# class BaseModel():
#     def __init__(self, name: str, features: list[str], hidden_layers: list[int], target : list[str], task : str, dataset_id: str):
#         self.name = name
...
#         self.dataset = Dataset.objects.get(id=dataset_id)
...
#     def train(self, X: pd.DataFrame, y: pd.DataFrame, epochs=20, batch_size=10):        
...
#                     name=self.name, user=self.dataset.user, dataset=self.dataset, model_file=binary_model,
